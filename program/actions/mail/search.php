...
class rcmail_action_mail_search extends rcmail_action_mail_index
{
    ...
    public function run($args = [])
    {
        ...
        // user value that reaches the SEARCH command unescaped; a newline in it could
        // inject extra IMAP commands. It MUST be sanitized before the command is
        // assembled — NOT after, because the search term is added as a length-counted
        // IMAP literal (escape(): {N}\r\n...) whose mandatory \r\n a whole-command
        // preg_replace would corrupt, breaking the literal and desyncing pipelined
        // multi-folder search on any accented term.
        $filter         = preg_replace('/[\r\n]+/', ' ', trim((string) $filter));
        ...
        // lives at the raw $filter input above, the only unescaped value in the
        // command; the search term is already safe via escape()'s counted literal.
        ...
        if (preg_match("/^(from|to|reply-to|cc|bcc|subject):.*/i", $str, $m)) {
            ...
            if ($headers) {
                foreach (explode(',', $headers) as $header) {
                    ...
                    case 'followupto':
                    case 'followup-to':
                        // AVUZ/Zoho: REPLY-TO, MAIL-REPLY-TO, FOLLOWUP-TO and
                        // MAIL-FOLLOWUP-TO are not indexed by Zoho's IMAP SEARCH.
                        // Zoho does not reject them — it returns an EMPTY result for
                        // the whole OR expression they appear in, so a single such
                        // term silently zeroes an otherwise-good subject/from search.
                        // They never match anything on Zoho regardless, so dropping
                        // them loses no real hits and un-poisons the OR. Skip them.
                        break;
                    default:
