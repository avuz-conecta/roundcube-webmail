...
class Actions_Mail_Search extends ActionTestCase
{
    ...
    function test_search_incomplete_renders_partial_results()
    {
        ...
        // the client already clears the list itself for the initial round;
        // a second server-issued clear here could race with it
        $this->assertFalse(strpos($result['exec'], 'this.message_list.clear(') !== false,
            'the initial (non-continuation) round must not emit an extra list clear');
    }

    /**
     * A continuation round re-lists the FULL accumulated cross-folder result,
     * not just the newest folder's hits. Since the client's continue_search()
     * never clears the list itself (unlike the initial qsearch), and
     * add_message_row() only dedupes-and-appends by uid, the server must
     * tell the client to clear the list before rendering the accumulated
     * rows, or the on-screen order degrades into arrival order.
     */
    function test_search_continuation_clears_list_before_rendering()
    {
        $action = new rcmail_action_mail_search;
        $output = $this->initOutput(rcmail_action::MODE_AJAX, 'mail', 'search');
        ...
        $clear_pos = strpos($result['exec'], 'this.message_list.clear(true);');
        $row_pos   = strpos($result['exec'], 'partial hit');

        $this->assertNotFalse($clear_pos,
            'a continuation round must clear the list before re-rendering the accumulated results');
        $this->assertNotFalse($row_pos,
            'the accumulated rows must still be rendered');
        $this->assertLessThan($row_pos, $clear_pos,
            'the list must be cleared BEFORE the rows are rendered, or the client would drop them');
    }
